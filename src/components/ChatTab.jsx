...
function ComingSoonScreen() {
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[400px] text-center px-4">
      <div className="w-20 h-20 rounded-full bg-gradient-gold flex items-center justify-center shadow-gold mb-6 relative">
        <Sparkles className="w-10 h-10 text-ink-900" />
        <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-ink-900 border-2 border-gold-500 flex items-center justify-center">
      ...
      <p className="text-sm text-ivory-100/70 max-w-sm leading-relaxed mb-6">
      ...
      <div className="card-warm max-w-sm w-full text-left">
        <h4 className="font-display font-semibold text-gold-500 mb-2 text-sm">
          ✨ What it will do
        </h4>
        <ul className="space-y-2 text-xs text-ivory-100/70">
      ...
      <p className="text-xs text-ivory-100/40 mt-6 font-accent italic">
...
Respond in English, concisely (up to 4 sentences when possible), in a friendly and warm tone. If the user asks about a specific place, restaurant, or attraction, answer in a focused way. When relevant, end with a suggestion: "Want me to add this to your itinerary?"`;
  ...
  return (
    <div className="animate-fade-in flex flex-col h-[calc(100vh-280px)] min-h-[500px]">
      <div className="flex-1 overflow-y-auto space-y-4 pb-4">
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-ivory-100/60 max-w-xs mx-auto leading-relaxed mb-6">
            ...
            <div className="space-y-2 max-w-sm mx-auto">
              {suggestions.map((s) => (
                <button
                  key={s}
                  onClick={() => setInput(s)}
                  className="w-full text-left px-4 py-2.5 bg-ink-800/40 border border-lacquer-900/30 rounded-xl text-sm text-ivory-100/80 hover:border-gold-500/30 transition-all"
                >
                  {s}
                </button>
              ))}
      ...
      <form onSubmit={sendMessage} className="flex gap-2 pt-3 border-t border-lacquer-900/30">
        <input
          type="text"
          value={input}
    ...
    <div className={cn('flex animate-fade-in', isUser ? 'justify-end' : 'justify-start')}>
      <div
        className={cn(
          ...
          isUser
            ? 'bg-gradient-lacquer text-ivory-50 rounded-br-md'
            : 'bg-ink-800/80 text-ivory-100 border border-gold-500/20 rounded-bl-md'
        )}
      >
        {!isUser && (
          <div className="flex items-center gap-1.5 mb-1 text-gold-500 text-xs font-accent italic">
